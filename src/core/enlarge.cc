                            ...
                            const TensorHandle _w2)
{
  ...
  //add one constraint
  assert((_w1->dim[2] != _w2->dim[2]) || (_w1->dim[3] != _w2->dim[3]));
  //end add
...
Op Model::get_or_create_enlarge(Tensor _w1, Tensor _w2)
{
  ...
  // now change this check because (1,3) and (3,1) can be both enlarged to (3,3)
  //if (_w1.dim[2] > _w2.dim[2] || _w1.dim[3] > _w2.dim[3])
  //  return Op::INVALID_OP;
  if ((_w1.dim[2] >= _w2.dim[2]) && (_w1.dim[3] >= _w2.dim[3]))
    return Op::INVALID_OP;
  // Check 3: w1 does not have the same size as w2
...
: OpBase(_w1, _w2, _model, OP_ENLARGE)
{
  assert(_w1.numDim == 4);
  assert(_w2.numDim == 4);
  //because (1,3) and (3,1) can be both enlarged to (3,3), change this check
  //assert(_w1.dim[2] <= _w2.dim[2]);
  //assert(_w1.dim[3] <= _w2.dim[3]);
  assert((_w1.dim[2] < _w2.dim[2]) || (_w1.dim[3] < _w2.dim[3]));
  //add one constraint
  ...
  // change this because (1,3) and (3,1) can be both enlarged to (3,3)
  //outputs[0].dim[2] = _w2.dim[2];
  //outputs[0].dim[3] = _w2.dim[3];
  if (_w1.dim[2] < _w2.dim[2])
	  outputs[0].dim[2] = _w2.dim[2];
  else
	  outputs[0].dim[2] = _w1.dim[2];
  if (_w1.dim[3] < _w2.dim[3])
	  outputs[0].dim[3] = _w2.dim[3];
  else
	  outputs[0].dim[3] = _w1.dim[3];
