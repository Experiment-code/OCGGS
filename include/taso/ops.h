...
//  size_t guid;
//  OpBase* ptr;
//  //int index; //the index of this op in the graph, only useful in the initial graph
//  //int par_i; //in which partition this op belongs to, valid in every graph
//};

//-----------THE NEW OP STRUCTURE FOR ALL SEARCH ALGORITHMS-------------------------//
struct Op {
	Op(void);
	Op(size_t _guid, OpBase* _ptr)
		: guid(_guid), ptr(_ptr)
	...
	size_t guid;
	OpBase* ptr;
	int SIStepOrder; //which optimization step in the sequence the operator is generated according to
	int SIidx; //which node in the target graph the node is mapped to
	int guid_for_cost; //this guid is for cost fixing, guid_for_cost is set to -1 at first, then we look at guid for its value
};
...
//--------------------------------THE STRUCTURE FOR ALL ALGORITHMS TO FIX THE COST----------------------------//
//////////////////code for compare two search tree nodes
struct SubstKey
{
	int substType; //which graph substitution (GraphXfer) is applied
	std::vector<size_t> srcOp_guids; //the guid of srcOps
};

class SubstKeyCompare {
public:
	bool operator() (SubstKey lhs, SubstKey rhs) {
		if (lhs.substType != rhs.substType)
			return lhs.substType < rhs.substType;
		//types are the same
		for (size_t i = 0; i < lhs.srcOp_guids.size(); i++) {
			if (lhs.srcOp_guids[i] != rhs.srcOp_guids[i])
				return lhs.srcOp_guids[i] < rhs.srcOp_guids[i];
		}
...
public:
  ...
  //Graph* optimize(float alpha, int budget, bool print_subst, int eraly_stop_num);
  Graph* optimize_prune(float alpha, int budget, bool print_subst);
  Graph* optimize_enumeration(float alpha, int budget, bool print_subst);
  Graph* optimize_reuse(float alpha, int budget, bool print_subst);
  Graph* optimize_sampletrick(float alpha, int budget, bool print_subst, int sample_size);
  Graph* optimize_sampletrick_local(float alpha, int budget, bool print_subst, int sample_size);
  Graph* optimize_sampletrick_newreuse(float alpha, int budget, bool print_subst, int sample_size);
  Graph* optimize_sampletrick_newreuse_2step(float alpha, int budget, bool print_subst, int sample_size);
  Graph* optimize_sampletrick_newreuse_2samplestep(float alpha, int budget, bool print_subst, int sample_size, bool do_weight_process);
  Graph* optimize_sampletrick_truenewreuse(int which_sample, float alpha, int budget, bool print_subst, int sample_size);
  Graph* optimize_partition(float alpha, int budget, bool print_subst, int eraly_stop_num, 
	  std::vector<std::vector<std::vector<int>>> partitions, bool do_weight_process);
  std::vector<std::vector<int>> get_vertex_weight();
  Graph* optimize_sysmltrick(float alpha, int budget, bool print_subst, int eraly_stop_num, bool do_weight_process);
...
public:
  ...
  struct GraphSubst {
	  std::vector<Op> srcOps, dstOps;
	  int substType; //which graph substitution (GraphXfer) is applied
					 //size_t guid; //global unique id of this optimization step
	  int order; //the order of this optimization step in the partial sequence
	  Op biggestNode; //which node in srcOps is the biggest according to the partial order
	  float cost_change; //the cost change brought by this step
  };
  std::vector<GraphSubst> subst_history;
  //store all operators in inEdge in vector so that they can be randomly accessed
  std::vector<Op> operators_for_access; //only valid or used in the initial graph for partition
  std::map<Edge, int, EdgeCompare> par_infor; //store the partition information, i.e., which partition each edge in this graph is in

  std::set<SubstKey, SubstKeyCompare> symmetric_subst_found; //store the substitutions which are by symmetric rule found on this graph only
};
...
public:
  ...
  //subst_dictionary: in order to fix the cost for the same op
  std::map<SubstKey, std::vector<Op>, SubstKeyCompare> op_dict;
