...
cdef class PyGraph:
    ...
    def get_vertex_weight(self):
        cdef vector[vector[int]] results = self.p_graph.get_vertex_weight()
        # for i in results:
        #     for j in i:
        #         print(j, end=', ')
        #     print('\n')
    ...
    def optimize_prune(self, float alpha, int budget, bool print_subst):
        cdef Graph* new_graph = self.p_graph.optimize_prune(alpha, budget, print_subst)
        graph = ctypes.cast(<unsigned long long>new_graph, ctypes.c_void_p)
        return PyGraph(graph)


    def optimize_enumeration(self, float alpha, int budget, bool print_subst):
        cdef Graph* new_graph = self.p_graph.optimize_enumeration(alpha, budget, print_subst)
        graph = ctypes.cast(<unsigned long long>new_graph, ctypes.c_void_p)
        return PyGraph(graph)
        

    def optimize_reuse(self, float alpha, int budget, bool print_subst):
        cdef Graph* new_graph = self.p_graph.optimize_reuse(alpha, budget, print_subst)
        graph = ctypes.cast(<unsigned long long>new_graph, ctypes.c_void_p)
        return PyGraph(graph)

    def optimize_sampletrick(self, float alpha, int budget, bool print_subst, int sample_size):
        cdef Graph* new_graph = self.p_graph.optimize_sampletrick(alpha, budget, print_subst, sample_size)
        graph = ctypes.cast(<unsigned long long>new_graph, ctypes.c_void_p)
        return PyGraph(graph)

    def optimize_sampletrick_local(self, float alpha, int budget, bool print_subst, int sample_size):
        cdef Graph* new_graph = self.p_graph.optimize_sampletrick_local(alpha, budget, print_subst, sample_size)
        graph = ctypes.cast(<unsigned long long>new_graph, ctypes.c_void_p)
        return PyGraph(graph)


    def optimize_sampletrick_newreuse(self, float alpha, int budget, bool print_subst, int sample_size):
        cdef Graph* new_graph = self.p_graph.optimize_sampletrick_newreuse(alpha, budget, print_subst, sample_size)
        graph = ctypes.cast(<unsigned long long>new_graph, ctypes.c_void_p)
        return PyGraph(graph)

    def optimize_sampletrick_newreuse_2step(self, float alpha, int budget, bool print_subst, int sample_size):
        cdef Graph* new_graph = self.p_graph.optimize_sampletrick_newreuse_2step(alpha, budget, print_subst, sample_size)
        graph = ctypes.cast(<unsigned long long>new_graph, ctypes.c_void_p)
        return PyGraph(graph)    

    def optimize_sampletrick_newreuse_2samplestep(self, float alpha, int budget, bool print_subst, int sample_size, bool do_weight_process):
        cdef Graph* new_graph = self.p_graph.optimize_sampletrick_newreuse_2samplestep(alpha, budget, print_subst, sample_size, do_weight_process)
        graph = ctypes.cast(<unsigned long long>new_graph, ctypes.c_void_p)
        return PyGraph(graph)    


    def optimize_sampletrick_truenewreuse(self, int which_sample, float alpha, int budget, bool print_subst, int sample_size):
        cdef Graph* new_graph = self.p_graph.optimize_sampletrick_truenewreuse(which_sample, alpha, budget, print_subst, sample_size)
        graph = ctypes.cast(<unsigned long long>new_graph, ctypes.c_void_p)
        return PyGraph(graph)

    def optimize_partition(self, float alpha, int budget, bool print_subst, int eraly_stop_num, partitions, bool do_weight_process):
        cdef vector[vector[vector[int]]] par_infor = partitions
        cdef Graph* new_graph = self.p_graph.optimize_partition(alpha, budget, print_subst, eraly_stop_num, par_infor, do_weight_process)
        graph = ctypes.cast(<unsigned long long>new_graph, ctypes.c_void_p)
        return PyGraph(graph)

    def optimize_sysmltrick(self, float alpha, int budget, bool print_subst, int eraly_stop_num, bool do_weight_process):
        cdef Graph* new_graph = self.p_graph.optimize_sysmltrick(alpha, budget, print_subst, eraly_stop_num, do_weight_process)
        graph = ctypes.cast(<unsigned long long>new_graph, ctypes.c_void_p)
        return PyGraph(graph)
